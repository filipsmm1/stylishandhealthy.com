...
(function() {
  ...
  // Only inject once, and only if hamburger doesn't already exist
...
// Newsletter form handler — posts to Mailchimp via fetch (email only list)
// Keeps your styling, no Mailchimp UI, no page redirect
...
function handleNwSub(e) {
  ...
  body.set(MC_HONEYPOT_NAME, ''); // honeypot — must be empty

  fetch(MC_NEWSLETTER_URL, {
    method: 'POST',
    mode: 'no-cors', // Mailchimp doesn't support CORS — no-cors is correct
    body: body,
  })
    .then(() => {
